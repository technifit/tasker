...
export const stripeRouter = createTRPCRouter({
  ...
  createSession: protectedProcedure.input(z.object({ planId: z.string() })).mutation(async (opts) => {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{ price: env.NEXT_PUBLIC_STRIPE_PRO_MONTHLY_PRICE_ID, quantity: 1 }],
    });
  ...
  plans: publicProcedure.query(async () => {
    const proPrice = await stripe.prices.retrieve(env.NEXT_PUBLIC_STRIPE_PRO_MONTHLY_PRICE_ID);
    const stdPrice = await stripe.prices.retrieve(env.NEXT_PUBLIC_STRIPE_STD_MONTHLY_PRICE_ID);
  ...
  purchaseOrg: protectedProcedure.input(purchaseOrgSchema).mutation(async (opts) => {
    const { userId } = opts.ctx.auth;
