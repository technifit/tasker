...
import { createApiKeySchema, createProjectSchema, renameProjectSchema, transferToOrgSchema } from '../../validators';
...
export const projectRouter = createTRPCRouter({
  create: protectedProcedure.input(createProjectSchema).mutation(async (opts) => {
    ...
    let query = opts.ctx.db.selectFrom('Project').select(({ fn }) => [fn.count<number>('id').as('projects')]);
    if (orgId) {
  ...
  delete: protectedProcedure.input(z.object({ id: z.string() })).mutation(async (opts) => {
    ...
    const deleteQuery = opts.ctx.db.deleteFrom('Project').where('id', '=', opts.input.id);
  ...
  transferToPersonal: protectedAdminProcedure.input(z.object({ id: z.string() })).mutation(async (opts) => {
  ...
  transferToOrganization: protectedProcedure.input(transferToOrgSchema).mutation(async (opts) => {
    ...
    const project = await opts.ctx.db
      ...
      .where(({ cmpr, and, or }) =>
        and([
          ...
          or([cmpr('userId', '=', userId), cmpr('organizationId', '=', userOrgId ?? '')]),
        ]),
      )
      .executeTakeFirst();
    ...
    if (project.organizationId && project.organizationId !== userOrgId && orgRole !== 'admin') {
      throw new TRPCError({
        code: 'UNAUTHORIZED',
  ...
  listByActiveWorkspace: protectedProcedure.query(async (opts) => {
    ...
    let query = opts.ctx.db.selectFrom('Project').select(['id', 'name', 'url', 'tier']);
    if (orgId) {
      query = query.where('organizationId', '=', orgId);
    } else {
      query = query.where('userId', '=', userId);
  ...
  byId: protectedProcedure.input(z.object({ id: z.string() })).query(async (opts) => {
    ...
    const query = opts.ctx.db
      ...
      .where(({ cmpr, and, or }) =>
        and([
          cmpr('id', '=', id),
          orgIds.length > 0
            ? or([cmpr('userId', '=', userId), cmpr('organizationId', 'in', orgIds)])
            : cmpr('userId', '=', userId),
        ]),
      );
  ...
  listApiKeys: protectedProcedure
    ...
    .query(async (opts) => {
      ...
      const apiKeys = await opts.ctx.db
        .selectFrom('ApiKey')
        .select(['id', 'name', 'key', 'createdAt', 'lastUsed', 'expiresAt', 'revokedAt'])
        ...
        .orderBy((eb) =>
          eb
            ...
            .then(3)
            .when(eb.and([eb.cmpr('expiresAt', 'is not', null), eb.cmpr('expiresAt', '<', new Date())]))
            .then(2)
            .else(1)
            .end(),
        )
  ...
  createApiKey: protectedProcedure.input(createApiKeySchema).mutation(async (opts) => {
    ...
    if (project.organizationId) {
      ...
      const isMemberInProjectOrg = orgs.some((org) => org.organization.id === project.organizationId);
  ...
  revokeApiKeys: protectedProcedure.input(z.object({ ids: z.string().array() })).mutation(async (opts) => {
    const { userId } = opts.ctx.auth;
  ...
  rollApiKey: protectedProcedure.input(z.object({ id: z.string() })).mutation(async (opts) => {
    ...
    await opts.ctx.db.updateTable('ApiKey').set({ key: newKey }).where('id', '=', opts.input.id).execute();
